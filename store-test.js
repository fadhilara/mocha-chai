...
describe('Stores Test', function() {
	...
	it('Success Create User Stores', function (done) {
		let name = 'John Doe Store', address = 'Panglima Sudirman 17', description = 'John Doe Store Panglima Sudirman 17', status='active'
	...
	it('Success Get User Stores', function (done) {
		...
		.end(function (err, res) {
			expect(res.status).to.equals(200);
			expect(res.body).to.be.jsonSchema( require('./auth/schema/success_get_store_schema.json'));
			if(err){
                    throw err;
                }
			done();
